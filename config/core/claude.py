...
class HybridizationPredictor:
    ...
    def create_pairs(self, sample_size=20000, balance_ratio=0.6):
        ...
        if 'C_value' in self.feature_cols and 'CV_C_value' in self.feature_cols:
            C_value_data = (self.df['C_value'].values, self.df['CV_C_value'].values)
        ...
        # Feature dimension should now be 83 (81 + 2 new C-value features)
...
╚══════════════════════════════════════════════════════════════════════╝
    ...
    X_pairs, y_pairs = predictor.create_pairs(sample_size=10000)
    ...
    if len(available) >= 3:
        print("\n" + "-"*70)
        predictor.predict(available[0], available[1540])
        predictor.predict(available[0], available[2])
        predictor.predict(available[1], available[4])
