# Load the complete class definition from the user's input
import pandas as pd
import numpy as np
...
class HybridizationPredictor:
    ...
    def create_pairs(self, sample_size=100000, balance_ratio=0.6):
        """
        ...
        # Get the unscaled/imputed pollination syndrome values
        pollination_values = None
        if 'pollination_syndrome' in self.feature_cols:
            pollination_values = self.df['pollination_syndrome'].values

        # Get the unscaled/imputed C-value data
        C_value_data = None
        if 'C_value' in self.feature_cols and 'CV_C_value' in self.feature_cols:
            C_values = self.df['C_value'].values
            CV_C_values = self.df['CV_C_value'].values
            C_value_data = (C_values, CV_C_values)
        ...
        while pairs_created < sample_size and attempts < max_attempts:
            ...
            # Extract features (scaled)
            f1 = self.X_scaled[i]
            f2 = self.X_scaled[j]
            
            # Feature engineering - standard set (uses scaled values)
            ...
            # START OF POLLINATION SYNDROME ENHANCEMENT (using unscaled/imputed values)
            pollination_features = []
            if pollination_values is not None:
                p1 = pollination_values[i]
                p2 = pollination_values[j]
                
                # Absolute difference of raw pollination values
                pollination_diff = np.abs(p1 - p2)
                # Product of raw pollination values (measure of co-occurrence/similarity)
                pollination_prod = p1 * p2
                
                pollination_features = [pollination_diff, pollination_prod]
            
            features = np.concatenate([features, pollination_features])
            # END OF POLLINATION SYNDROME ENHANCEMENT

            # START OF C-VALUE ENHANCEMENT (using unscaled/imputed values)
            c_value_features = []
            if C_value_data is not None:
                C_values, CV_C_values = C_value_data
                
                c1, cv_c1 = C_values[i], CV_C_values[i]
                c2, cv_c2 = C_values[j], CV_C_values[j]
                
                # Absolute difference of raw C-values (major constraint for hybridization)
                C_value_diff = np.abs(c1 - c2)
                # Absolute difference of raw CV_C-values (difference in genome size variation)
                CV_C_value_diff = np.abs(cv_c1 - cv_c2)
                
                c_value_features = [C_value_diff, CV_C_value_diff]
            
            features = np.concatenate([features, c_value_features])
            # END OF C-VALUE ENHANCEMENT
        ...
        # Feature dimension should be 6 * 11 (standard features) + 2 (pollination) + 2 (C-value) + 2 (taxonomic) = 70
        # Wait, the feature set is: ['perc_per', 'perc_wood', 'perc_ag', 'floral_symm', 'mating_system', 'repro_syndrome', 'pollination_syndrome', 'RedList', 'C_value', 'CV_C_value', 'tavg'] = 11 features.
        # Standard features: 6 * 11 = 66
        # Pollination enhancements: 2
        # C-value enhancements: 2
        # Taxonomic similarity: 2 (Family, Order)
        # Total: 66 + 2 + 2 + 2 = 72 features. The previous comment in the user's code was 83, which seems incorrect given 11 base features. Let's check the actual shape.
    ...
    def predict(self, plant1, plant2, verbose=True):
        ...
        # Base features (uses scaled values)
        concat = np.concatenate([f1, f2])
        diff = np.abs(f1 - f2)
        prod = f1 * f2
        mean = (f1 + f2) / 2
        ...
        # START OF POLLINATION SYNDROME ENHANCEMENT
        pollination_features = []
        if 'pollination_syndrome' in self.feature_cols:
            pollination_values = self.df['pollination_syndrome'].values
            p1 = pollination_values[i]
...
# Main execution logic as provided by the user
...
# The user's original main execution logic continues below, modified slightly to fit the environment.
print("\n" + "="*70)
...
if len(available) >= 3:
    print("\n" + "-"*70)
    # Use the first three available plants for prediction examples
